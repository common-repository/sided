...
.content-wrapper1 .content{
  padding: 10px !important;
}
html.wp-toolbar{
  overflow:  hidden !important;
}
.block-create-debate-btns{
  text-align: center;
  margin-top: 25px;
}
...
if(isset($_POST['thesis'])){
  ...
  if( !is_wp_error( $response ) ) {
    ...
    if($response_arr->status == 'success'){
      ...
      echo '<script>window.parent.postMessage({"did" : "'.esc_js($response_arr->data->id).'"}, "*");</script>';
      die;
    } else {
      echo '<h3 style="text-align:center !important; color: red !important;">'.esc_js($response_arr->message[0]->error).'</h3>'; die;
    }
  } else {
...
if($('#createDebate').outerWidth() < 800) {
  $('#createDebate').addClass('singleCol');
}
...
<div class="content-wrapper1 createDebateFromBlock">
  <div class="content">
  <form id="createDebate" class="startDebate common-form row" action="" method="POST" enctype="multipart/form-data">
    ...
    <input id="typeField" type="hidden" name="type" value="create">
            ...
            <select data-required="true" name="categoryId" class="categoryId">
               ...
               <?php
                foreach($debate_cats as $debate_cat){
                  echo '<option value="'.esc_attr($debate_cat->id).'" data-id="'.esc_attr($debate_cat->id).'">'.esc_attr($debate_cat->text).'</option>';
                }
               ?>
            </select>
            <textarea class=" mt-2 form-control" name="background" placeholder="Share any information that might help people choose a side"></textarea>
         </div>
      </div>
      <div class="col-md-6 second-col">
         <div class="inline-content">
            <div class="me-3 pt-0 form-group w-50">
            ...
            <div class="pt-0 form-group w-50">
         ...
         <div class="form-group">
          <label class="form-label">Poll Author</label>
          <select name="userId">
      ...
      <div class="block-create-debate-btns">
            <button type="submit" class="mt-0 me-0 btn btn-primary">Publish Poll</button>
      </div>
   </form>
  </div>
</div>